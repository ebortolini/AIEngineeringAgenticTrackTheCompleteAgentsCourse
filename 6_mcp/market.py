...
def _make_reference_client():
    if ReferenceClient is not None:
        return ReferenceClient(polygon_api_key)
    return RESTClient(polygon_api_key)


def _make_stocks_client():
    if StocksClient is not None:
        return StocksClient(polygon_api_key)
    return RESTClient(polygon_api_key)


def is_market_open() -> bool:
    client = _make_reference_client()
